...
namespace protocol {
    ...
    static constexpr long TXN_TIMEOUT_MS = 30000;
    ...
    inline Request parse(const std::string& line) {
        ...
        else if (verb == "BEGIN") req.command = Command::BEGIN;
        else if (verb == "COMMIT") req.command = Command::COMMIT;
        else if (verb == "ABORT") req.command = Command::ABORT;
        else if (verb == "SHUTDOWN") req.command = Command::SHUTDOWN;
